...
def switch_content(prediction_clicks, watchlist_clicks, current_layout):
    ...
    elif triggered_id == 'watchlist_button':
...
def predict_stock_price(n_clicks, stock_symbols, start_date, end_date):
    ...
    graph_data = []
    prediction_details = []
    scaler = MinMaxScaler(feature_range=(0, 1))  # Initializing scaler inside the function
    for index, symbol in enumerate(symbols):
        ...
        X_test = np.array([inputs[i-60:i] for i in range(60, len(inputs))])
        predicted_prices = model.predict(X_test)
        predicted_prices = scaler.inverse_transform(np.hstack((predicted_prices, np.zeros((len(predicted_prices), inputs.shape[1]-1)))))[:, 0]

        # Predict the next trading day's price
        X_next_day = np.array([inputs[-60:]])
        next_day_predicted_price = model.predict(X_next_day)
        dummy = np.zeros((1, inputs.shape[1]))
        dummy[:, 0] = next_day_predicted_price[:, 0]
        next_day_predicted_price = scaler.inverse_transform(dummy)[0, 0]

        # Append next day prediction
        predicted_dates = data.index[60:]
        predicted_dates = predicted_dates.append(pd.Index([predicted_dates[-1] + pd.Timedelta(days=1)]))
        predicted_prices = np.append(predicted_prices, next_day_predicted_price)
        ...
        graph_data.append(dcc.Graph(
            ...
            figure={
                'data': [
                     {'x': data.index[60:], 'y': data['Close'][60:], 'type': 'line', 'name': 'Actual Price', 'line': {'color': 'blue'}},
                     {'x': data.index[60:], 'y': predicted_prices[:-1], 'type': 'line', 'name': 'Predicted Price', 'line': {'color': 'green'}},
                    {'x': [data.index[-1] + pd.Timedelta(days=1)], 'y': [next_day_predicted_price], 'type': 'markers', 'name': 'Next Day Prediction', 'marker': {'color': 'red', 'size': 8}}
                ],
                'layout': {
        ...
        prediction_details.append(html.P(f"Predicted close for next trading day ({predicted_dates[-1].date()}): ${next_day_predicted_price:.2f}"))
